...
N,denom =list(map(int,input().split()))
num=[]
for _ in range(N):
    garb_val ,*arr=list(map(int,input().split()))
    num.append(arr)

#map of map
sq=list(map(lambda x: list(map(lambda y: y**2,x)),num))
prod=product(*sq)
print(max(list(map(lambda x: sum(x)%denom,prod))))
