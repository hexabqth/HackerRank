...
students,subjects=tuple(map(int,input().split()))
data=[list(map(float,input().split())) for _ in range(subjects)]
average=[format(sum([data[d][_] for d in range(subjects)])/subjects,".2f") for _ in range(students)]
print(*average,sep='\n')
